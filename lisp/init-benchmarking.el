...
;; refector the init-time message to be startup-echo-area-message.
(defun sanityinc/startup-echo-area-message ()
  (if (daemonp) "Starting Emacs daemon."
    (format "init completed in %.2fms"
            (sanityinc/time-subtract-millis after-init-time before-init-time))))

(fset 'startup-echo-area-message 'sanityinc/startup-echo-area-message)
