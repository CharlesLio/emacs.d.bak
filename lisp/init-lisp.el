...
(use-package elisp-format :ensure t :defer t)

;;; Read something thoughtful
;; cargo install fsays
(defvar cowsay-scratch-message
  (shell-command-to-string
   (let* ((random-bin (seq-random-elt '("cowsay" "cowthink" "fsays")))
          (cowsay-option (unless (string-equal random-bin "fsays")
                           (seq-random-elt '("-b" "-d" "-g" "-p" "-t" "-y")))))
     (string-join (list random-bin cowsay-option "$(fortune) | sed 's/^/;; /;s/[[:blank:]]*$//'") " ")))
  "cowsay fortune message for `*scratch*' buffer")

(setq-default initial-scratch-message
              (if (and (executable-find "cowsay") (executable-find "fortune"))
                  (format "%s\n" cowsay-scratch-message)
                (format ";; Happy hacking, %s!\n" user-full-name)))

;; change <F10> key binding, never used default `menu-bar-open' anyway
(global-set-key [remap menu-bar-open] 'scratch-buffer)
...
(defun sanityinc/eval-last-sexp-or-region (prefix)
  "Eval region from BEG to END if active, otherwise the last sexp of PREFIX."
  (interactive "P")
...
(defun sanityinc/make-read-only (expression out-buffer-name &optional lisp)
...
(use-package immortal-scratch :ensure t
  :config ;; activate immortal-scratch-mode after startup
  (add-hook 'after-init-hook 'immortal-scratch-mode))

;; go to `point-max' in scratch buffer when startup and insert a `newline'
(defun scratch-buffer-eol ()
  (interactive)
  (with-current-buffer "*scratch*"
    (goto-char (point-max))
    (set-mark (point-max))
    (deactivate-mark t)))
(add-hook 'after-init-hook #'scratch-buffer-eol)
...
(add-hook 'emacs-lisp-mode-hook #'global-dash-fontify-mode)
(add-hook 'lisp-interaction-mode-hook #'global-dash-fontify-mode)

...
(defun sanityinc/reverting (orig &rest args)
  "Apply ORIG to ARGS when SANITYINC/VC-REVERTING is true."
...
(with-eval-after-load 'lisp-mode
  ...
  (define-key lisp-interaction-mode-map (kbd "C-c x") 'macrostep-expand))
...
;; enable live reloading of dynamic modules
(defun module-reload-fake (module)
